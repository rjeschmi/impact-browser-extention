...
import { getOllamaModel } from "../services/ollama.js";
...
// --- General Settings ---
app.patch("/", async (c) => {
	const body = (await c.req.json()) as { ollamaModel?: string };
	if (body.ollamaModel) {
		const now = Date.now();
		const existing = db
			.select()
			.from(schema.appSettings)
			.where(eq(schema.appSettings.key, "ollama_model"))
			.get();
		if (existing) {
			db.update(schema.appSettings)
				.set({ value: body.ollamaModel, updatedAt: now })
				.where(eq(schema.appSettings.key, "ollama_model"))
				.run();
		} else {
			db.insert(schema.appSettings)
				.values({ key: "ollama_model", value: body.ollamaModel, updatedAt: now })
				.run();
		}
	}
...
app.get("/export", (c) => {
	...
	c.header(
		"Content-Disposition",
		`attachment; filename="impact-export-${Date.now()}.json"`,
	);
	return c.json({
		exportedAt: Date.now(),
		visits,
...
app.delete("/purge", async (c) => {
	const { olderThanDays } = z
		.object({ olderThanDays: z.number().int().min(1).max(365) })
		.parse(await c.req.json());

	const cutoff = Date.now() - olderThanDays * 86400000;

	const deletedVisits = db
		.delete(schema.pageVisits)
		.where(lt(schema.pageVisits.visitedAt, cutoff))
		.returning({ id: schema.pageVisits.id })
		.all();

	const deletedExtractions = db
	...
	return c.json({
		deleted: {
			...
			extractions: deletedExtractions.length,
		},
		cutoff,
	});
});
